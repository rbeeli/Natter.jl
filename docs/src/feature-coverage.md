...
| Flow control / heartbeats | Supported | Pull fetch consumes status/control messages and monitors requested idle heartbeats. Non-queue push consumers hide idle heartbeats, report missed heartbeats and consumer sequence mismatches through `error_cb`, and answer flow-control requests plus stalled-consumer heartbeats internally. |
