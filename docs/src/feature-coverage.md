...
```bash
env NATTER_RUN_INTEGRATION=true NATTER_RUN_JETSTREAM=true NATTER_RUN_STRESS=true NATTER_STRESS_SECONDS=45.0 NATTER_STRESS_KEYS=2000 NATTER_STRESS_WATCHERS=4 julia --project=. -e 'using Pkg; Pkg.test()'
```

The `Stress` GitHub Actions workflow runs this live-server stress suite daily and by manual dispatch.
