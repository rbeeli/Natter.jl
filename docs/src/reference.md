...
| `fetch(psub, batch=1; timeout=..., expires=<shorter than timeout>, heartbeat=nothing, max_bytes=nothing, no_wait=false)` | Fetch a bounded batch of `JetStreamMsg` values. `max_bytes` adds a server byte cap, and `no_wait=true` returns immediately with currently available messages. Fetches on one subscription are serialized, use unique reply subjects, ignore stale terminal statuses, and are not replayed after reconnect. |
| `messages(psub; batch=100, max_bytes=nothing, expires=30.0, heartbeat=nothing, threshold_messages=nothing, threshold_bytes=nothing, channel_size=batch, stop_after=nothing)` | Start a threshold-refilled pull message stream and return `PullMessageStream`. The stream supports iteration, `take!`, `wait`, `fetch`, and `close`. Only one active fetch or stream may use a pull subscription at a time. |
