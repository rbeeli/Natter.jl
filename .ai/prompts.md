do a full deep review of this pure Julia NATS client. look for performance issues, type stability issues, unneccessary allocations, easy to do optimizations, unneccessary branchy code etc.
    this should be a fast and efficient library, especially on the hot path. advise then on how to improve/fix the found issues. only report major findings, but report all of them.
